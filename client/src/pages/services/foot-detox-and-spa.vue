			...
			<BRow>
				<BCol cols="12">
					<h1 class="mb-3 text-center text-primary">
						{{ pData.services[3].title }}
					</h1>
				</BCol>

				<BCol cols="12" md="3" class="d-none d-md-block">
					<img :src="pData.services[3].image" class="w-100 mb-3 rounded">
				</BCol>

				<BCol cols="12" md="9">
					<p>{{ pData.services[3].description }}</p>

					<div v-if="pData.services[3].processSteps.length > 0">
						<h5 class="text-info">Session Process</h5>
						<ul>
							<li
								v-for="(p, i) in pData.services[3].processSteps"
								:key="i"
							>{{ p }}</li>
						</ul>
					</div>
